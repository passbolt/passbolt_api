...
class CategoriesControllerTest extends ControllerTestCase {
	...
	public function testGet() {
		...
		$result = json_decode($this->testAction("/categories/get/$id/1", array('return'=>'contents')), true);
		// test Anjuna id
		$this->assertEquals('4ff6111c-8534-4d17-869c-2184cbdd56cb', $result['body'][0]['children'][0]['children'][0]['Category']['id'],
			'The test should return anjuna but is returning ' . $result['body'][0]['children'][0]['children'][0]['Category']['name']
		);
	...
	public function testGetChildren() {
		...
		// test it is the expected format
		$result = json_decode($this->testAction("/categories/getChildren/$id", array('return'=>'contents')), true);
		$this->assertInternalType('array', $result['body']);

		// test that content returned are correct 
		$result = json_decode($this->testAction("/categories/getChildren/$id", array('return'=>'contents')), true);
		// test Anjuna 
		$this->assertEquals('Anjuna', $result['body'][0]['children'][0]['Category']['name']);
	...
	public function testAdd() {
		...
		$this->assertEquals('Aramboooool', $result['body']['Category']['name']);
	...
	public function testMove(){
		...
		$this->assertEquals(Message::ERROR, $result['header']['status']); // test if response is a success

		// test firstPosition
		$url = "/categories/move/{$testCases['firstPosition']['id']}/{$testCases['firstPosition']['position']}";
		$result = json_decode($this->testAction($url, array('return'=>'contents')), true);
		$this->assertEquals(Message::SUCCESS, $result['header']['status']); // test if response is a success
		...
		$result = json_decode($this->testAction($url, array('return'=>'contents')), true);
		$this->assertEquals(Message::ERROR, $result['header']['status']); // test if response is an error
