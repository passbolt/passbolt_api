		...
		replace= function(oldNodeList, newNodes){
			...
			can.each(oldNodeList, function(node){
				...
				can.each( can.makeArray( nodeMap[id(node)] ), function( nodeListId ){
					...
					if(startIndex >=0){
						//
						for( var i = startIndex; i < startIndex + oldNodeList.length; i++){
							var n = nodeList[i];
							removeNodeListId(n, nodeListId)
						}
					}
					// swap in new nodes into the nodeLIst
					nodeList.splice.apply(nodeList, [startIndex,oldNodeList.length   ].concat(newNodes))
	...
	extend(EJS, {
		...
		txt: function(escape, tagName, status, self, func){
			...
			var	parentElement,
				...
				setupTeardownOnDestroy = function(el, nl){
					nodeList = nl;
					...
					function( span, parentNode ) {
						...
						update = function(newVal){
							...
							if( attached ) {
								...
								console.log("NO longer attached!")
							}
						...
						var nodes = [span],
							makeAndPut = function(val){
								...
								console.log("removing", oldNodes)
								can.remove( can.$(oldNodes) );
								/*if( !nodes ) {
									can.remove( can.$(span) );
									nodes = newNodes;
									...
									console.log("replace",nodes, newNodes, val)
									replace(nodes,newNodes);
								}*/
							};
						...
						register(nodes)
						makeAndPut(binding.value);
						
						
						setupTeardownOnDestroy( parentNode, nodes );
