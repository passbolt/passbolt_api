...
class AccountRecoveryOrganizationPolicyEnableEmailRedactor extends AbstractAccountRecoveryOrganizationPolicyEmailRedactor implements SubscribedEmailRedactorInterface // phpcs:ignore
{
    ...
    {
        return [
            AbstractAccountRecoveryOrganizationPolicySetService::AFTER_ENABLE_POLICY_EVENT,
        ];
    }
    ...
    protected function makeAdminEmail(User $admin, User $user, AccountRecoveryOrganizationPolicy $policy): Email
    {
        ...
        $subject = (new LocaleService())->translateString(
            ...
            function () use ($admin, $user, $policy) {
                if ($admin->id === $user->id) {
                    return __('You have set the account recovery organization policy to {0}.', $policy->policy);
                }

                return __(
                    '{0} has set the account recovery organization policy to {1}.',
